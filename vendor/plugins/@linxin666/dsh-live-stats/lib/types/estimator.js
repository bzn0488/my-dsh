 ...
 */
export function resolveEstimatorConfig(config) {
    const known = new Set(['charsPerToken', 'blockOverhead', 'roleOverhead']);
    for (const key of Object.keys(config)) {
        if (!known.has(key))
            throw new Error(`live-stats: unknown config key "${key}"`);
    }
    const spec = {
    ...
    for (const key of ['blockOverhead', 'roleOverhead']) {
 ...
 */
export function estimateTextBlockTokens(characters, spec) {
 ...
 * @returns the estimated token count.
 */
export function estimateToolCallBlockTokens(nameCharacters, argumentCharacters, spec) {
 ...
 */
export function estimateAssistantBlockTokens(blockTokens, spec) {
 ...
 * @returns the estimated token count.
 */
export function estimateContentTokens(blocks, spec) {
 ...
 * @returns the estimated token count.
 */
export function estimateMessageTokens(message, spec) {
 ...
 */
export function estimateHeaderTokens(header, spec) {
    if (header === undefined)
        return 0;
    let tokens = 0;
