 ...
 */
export function extractBlockedPaths(stderr, header) {
    const start = stderr.indexOf('\n', stderr.search(header));
    if (start === -1)
        return { paths: [], moreFiles: 0 };
    const paths = [];
 ...
 */
export function classifySwitchFailure(stderr) {
